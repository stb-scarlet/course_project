...
async function findOrCreateOAuthUser(data) {
    ...
    const existing = await prisma_1.default.oAuthAccount.findUnique({
    ...
    let user = await prisma_1.default.user.findUnique({ where: { email: data.email } });
    if (!user) {
    ...
    // Link OAuth account
    await prisma_1.default.oAuthAccount.create({
        data: {
            userId: user.id,
